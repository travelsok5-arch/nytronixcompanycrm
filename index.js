...
const { db, backupDatabase, initializeDatabase } = require('./database');
const fs = require('fs');
...
app.post('/api/backups/upload-restore', requireAdmin, backupUpload.single('backupFile'), (req, res) => {
    try {
        if (!req.file) {
        ...
        const currentDbPath = path.join(__dirname, 'cybernytronix.db');
        ...
        db.close((closeErr) => {
            ...
            try {
                ...
                res.json({ 
                    success: true,
                    message: 'Backup uploaded and restored successfully. System will reload with restored data.'
                });
            } catch (copyError) {
                console.error('Error restoring backup:', copyError);
...
app.post('/api/backups/restore', requireAdmin, (req, res) => {
    ...
    db.close((err) => {
        ...
        try {
            ...
            res.json({ 
                success: true,
                message: 'Backup restored successfully. System will reload with restored data.'
            });
        } catch (copyError) {
            console.error('Error restoring backup:', copyError);
            res.status(500).json({ error: 'Failed to restore backup' });
...
app.listen(PORT, () => {
    ...
    console.log(`   ✅ Fixed "Unexpected end of form" errors`);
